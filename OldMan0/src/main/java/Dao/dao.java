...
public class dao {
    public void showall(ArrayList<bean01> book,String name,String phone) throws ClassNotFoundException, SQLException {

        String sql1= "select * from tigong ";

        if (name == null && phone!=null){
            System.out.println("1************************************");
            sql1="select * from tigong where phone like "+ phone;
        }
        if (phone == null && name != null) {
            System.out.println("2************************************");
             sql1="select * from tigong where name like "+ name;
        }
        if (name != null && phone != null)
        {
            System.out.println("3************************************");
            sql1="select * from tigong where name like "+ name+" phone like "+ phone;
        }
...
//            A.setRound(rs.getString(4));
    ...
    public void updateT(bean01 a) throws ClassNotFoundException, SQLException{
        Connection connection = Util.util.getConnection();
        String sql = " update tigong set guanxi = ?,name = ?,phone=? , tigongname = ? where IDcard = ?" ;
        PreparedStatement preparedStatement = null;//载体
        try {
...
//            A.setRound(rs.getString(4));
    ...
    public void sel99(ArrayList<bean03> book,String name) throws ClassNotFoundException, SQLException {
        ...
        String sql1="SELECT pnum, pdata, pyuanyin, name, idcard, richang, jingshen, ganzhi, shehui FROM pin where name like  ? ";
//        String sql1="SELECT * FROM text3_1 ORDER BY RAND() LIMIT 100";
        PreparedStatement pre=null;
        PreparedStatement ps=connection.prepareStatement(sql1);
        name = "%"+name+"%";
